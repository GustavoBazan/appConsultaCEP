...
export class HomePage {
  ...
  cepInfo: Observable<any> | undefined;
  ...
  consultarCEP(cep:string):void {
    this.cepInfo = this.http.get('https://viacep.com.br/ws/' + cep + '/json/').pipe(catchError(erro => this.exibirErro(erro)));
    this.cepInfo.subscribe((x) => {this.rua = x.logradouro});
    this.cepInfo.subscribe((x) => {this.bairro = x.bairro});
    this.cepInfo.subscribe((x) => {this.cidade = x.localidade});
    this.cepInfo.subscribe((x) => {this.estado = x.uf});
  }
